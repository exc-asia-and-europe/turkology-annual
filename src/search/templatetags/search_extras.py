...
class AddParameter(template.Node):
  def __init__(self, bits):
	...
	for i in xrange(0, len(bits), 2):
		if bits[i+1][0] == "+":
			self.addParamSet.append(len(self.args))
			bits[i+1] = bits[i+1][1:]
		self.args.append((template.Variable(bits[i]), template.Variable(bits[i+1])))

  def render(self, context):
	req = template.Variable('request').resolve(context)
	params = req.GET.copy()
	filtersNum = len([arg for arg in params if arg.startswith("filter_name")])
	for i in xrange(len(self.args)):
		arg = self.args[i][0]
		if arg.literal == "filter_name":
			arg.literal = "filter_name%d"%(filtersNum + 1)
		elif arg.literal == "filter_value":
			arg.literal = "filter_value%d"%(filtersNum + 1)

		val = self.args[i][1]
		if i in self.addParamSet:
			params[arg.resolve(context)] = "%s %s"%(params[arg.resolve(context)], val.resolve(context))
		else:
			params[arg.resolve(context)] = val.resolve(context)
...
def addurlparameter(parser, token):
  #bits = re.split(r'\s+', token.contents, 2)
  #print token.contents
  bits = token.contents.strip().split()[1:]
  if len(bits) < 2 or len(bits)%2 != 0:
	raise template.TemplateSyntaxError, "'%s' tag requires an even number (at least two) of arguments" % bits[0]
  return AddParameter(bits)
