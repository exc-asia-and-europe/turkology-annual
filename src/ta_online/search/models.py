...
def init(engine, create=False, non_primary=False):
...
def _map_tables(metadata, engine, non_primary):
    ...
    collectionArticles = Table('collectionArticles', metadata,
                               Column('articleId', Integer, ForeignKey('articles.id'), primary_key=True),
                               Column('collectionId', Integer, ForeignKey('entries.id')))
    ...
    idx_persons = Table('idx_persons', metadata,
                        Column('id', Integer, primary_key=True),
                        Column('name', String))
    ...
    idx_entryPersons = Table('idx_entryPersons', metadata,
                             Column('entryId', Integer, ForeignKey('entries.id'), primary_key=True,
                                    index=True),
                             Column('personId', Integer, ForeignKey('idx_persons.id'),
                                    primary_key=True, index=True),
    ...
    languageMapper(Article, articles, inherits=entryMapper,
            ...
            '_repetitions': relationship(EntryEntry,
                                         ...
                                         collection_class=ordering_list('position'),
                                         order_by=[entryEntries.c.position]),
            '_collections': relationship(CollectionArticle,
                                         primaryjoin=articles.c.id == collectionArticles.c.articleId),
        }, non_primary=non_primary)
    ...
    languageMapper(Collection, collections, inherits=entryMapper,
            ...
            '_repetitions': relationship(EntryEntry,
                                         ...
                                         collection_class=ordering_list('position'),
                                         order_by=[entryEntries.c.position]),
            '_articles': relationship(CollectionArticle,
                                      primaryjoin=entries.c.id == collectionArticles.c.collectionId,
                                      collection_class=ordering_list('articleId'),
                                      order_by=[collectionArticles.c.articleId]),
        }, non_primary=non_primary)
    ...
    mapper(IdxPerson, idx_persons, properties={
    }, non_primary=non_primary)  # XXX
    ...
    mapper(IdxEntryPerson, idx_entryPersons, properties={
        'entry': relationship(Entry),
        'person': relationship(IdxPerson)}, non_primary=non_primary)
    ...
    mapper(CollectionArticle, collectionArticles, properties={
        'collection': relationship(Entry,
                                   primaryjoin=collectionArticles.c.collectionId == entries.c.id),
        'article': relationship(Article,
                                primaryjoin=collectionArticles.c.articleId == articles.c.id)}, non_primary=non_primary)
...
class CollectionArticle(object):
...
class Collection(Entry):
    ...
    repetitions = association_proxy('_repetitions', 'sourceEntry')
    articles = association_proxy('_articles', 'article')
    categories = association_proxy('_categories', 'category')

    def __init__(self, volume, number, title, editors, volumes, cities,
                 country, year, paginations, comment, category, subEntries, raw,
                 problem, scanPage, scanPosition1, scanPosition2, categories, repetitions):
        self.volume = volume
        self.number = number
        self.title = languageString(title)
...
class IdxEntryPerson(object):
    def __init__(self, person, entry, role):
        self.person = person
        self.entry = entry
        self.role = role
...
class ConferenceDate(object):
    ...
    def __unicode__(self):
        if self.startDay == None and self.endDay == None:
            ...
            if self.startYear != self.endYear:
                return '%s.%s-%s.%s' % (self.startMonth,
                                        self.startYear, self.endMonth, self.endYear)
            if self.startMonth != self.endMonth:
                return '%s.-%s.%s' % (self.startMonth,
                                      self.endMonth, self.startYear)
            return '%s.%s' % (self.startMonth, self.startYear)

        # year, month and day given
        if self.startYear != self.endYear:
            return '%s.%s.%s-%s.%s.%s' % (self.startDay,
                                          self.startMonth, self.startYear, self.endDay,
                                          self.endMonth, self.endYear)
        if self.startMonth != self.endMonth:
            return '%s.%s.-%s.%s.%s' % (self.startDay,
                                        self.startMonth, self.endDay,
                                        self.endMonth, self.startYear)
        if self.startDay != self.endDay:
            return '%s.-%s.%s.%s' % (self.startDay, self.endDay,
                                     self.startMonth, self.startYear)
        return '%s.%s.%s' % (self.startDay, self.startMonth,
                             self.startYear)
